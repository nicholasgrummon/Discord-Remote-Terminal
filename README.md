...
<br>
1. Michelle: Operates on the host server. Run via `python michelle_discord.py` or `$ ./runner.sh` to decouple with terminal. Executes messages from discord as commands on the host server terminal and returns any stdout back to the discord channel. Accepts special commands as follows:
   1. `!ping` returns "pong!"
...
`$ python -m pip install -U stockfish`
